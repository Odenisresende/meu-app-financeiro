...
COMMENT ON COLUMN user_subscriptions.user_id IS 'ID do usuário (pode ser email ou ID do auth)';
COMMENT ON COLUMN user_subscriptions.subscription_id IS 'ID da assinatura do Mercado Pago';
COMMENT ON COLUMN user_subscriptions.preference_id IS 'ID da preferência de pagamento';
COMMENT ON COLUMN user_subscriptions.status IS 'Status: pending, active, cancelled, expired';
COMMENT ON COLUMN user_subscriptions.is_active IS 'Se a assinatura está ativa no momento';
